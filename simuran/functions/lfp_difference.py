# TODO this should do the lfp difference calculations

# 1. The colormap for each recording.

# 2. The average colormap for each rat.

# 3. A csv file with the average difference between RSC and SUB
# Between each rat.
...
def setup_functions():
    ...
    def argument_handler(recording_container, idx, figures):
        ...
        arguments = {
            "compare_lfp": (
                [figures, recording_container.output_dir, recording_container.base_dir],
                {"save_result": False, "plot": False},
            ),
            "average_difference": ([figures], {"plot": True, "fmt": "svg"}),
        }
        return arguments
...
def setup_output():
    ...
    save_list = [("results", "compare_lfp", "summary")]
